...
function Map({ lat, long, allListings, current = null }: SelectLocationProps) {
    let listings: ListingType[] | [] | undefined;
    if (current === null) {
        listings = []
    } else {

        listings = allListings?.filter((l) => l?.id !== current?.id);
    }
    ...
    return (
        <MapContainer center={[lat, long]} zoom={13} scrollWheelZoom={true} className='w-full h-full z-0 relative'>
            ...
            {current !== null ?
                <Marker icon={ICON} position={[current?.latitude as number, current?.longitude as number]} >
                    <Popup className='p-0'>
